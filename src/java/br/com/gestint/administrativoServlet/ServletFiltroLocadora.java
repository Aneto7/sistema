 ...
 * @author rosaan
 */
public class ServletFiltroLocadora extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, LOCADOR FROM `veiculos` UNION SELECT MIN(ID) AS ID, LOCADORA AS LOCADOR FROM `infracoes` WHERE "
                    + "`PLACA` LIKE '%" + nomeplaca
                    + "' GROUP BY LOCADOR ORDER BY LOCADOR");
        ...
        for (int i = 0; i < locadoras.size(); i++) {
            sb.append(locadoras.get(i).getId() + "-" + locadoras.get(i).getLocadora()+ ";");
        }
