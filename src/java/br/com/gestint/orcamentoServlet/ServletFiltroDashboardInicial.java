 ...
 * @author rosaan
 */
public class ServletFiltroDashboardInicial extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String nomecifexp = request.getParameter("idcifexp");
        String nomecifrec = request.getParameter("idcifrec");
        String nomefob = request.getParameter("idfob");
        if ("false".equals(nomecifexp)) {
            nomecifexp = "";
        } else {
            nomecifexp = "CIF EXP";
        }
        if ("false".equals(nomecifrec)) {
            nomecifrec = "";
        } else {
            nomecifrec = "CIF REC";
        }
        if ("false".equals(nomefob)) {
            nomefob = "";
        } else {
            nomefob = "FOB";
        }
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, SUM(IF((MES='JAN' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_JAN, SUM(IF((MES='FEV' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_FEV, SUM(IF((MES='MAR' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_MAR, SUM(IF((MES='ABR' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_ABR, SUM(IF((MES='MAI' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_MAI, SUM(IF((MES='JUN' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_JUN, SUM(IF((MES='JUL' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_JUL, SUM(IF((MES='AGO' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_AGO, SUM(IF((MES='SET' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_SET, SUM(IF((MES='OUT' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_OUT, SUM(IF((MES='NOV' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_NOV, SUM(IF((MES='DEZ' AND TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN_DEZ, SUM(IF((TIPO='VOLUME' AND PLANOUREAL='PLANEJADO'), VALOR, 0)) AS VOLUMEPLAN, SUM(IF((MES='JAN' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_JAN, SUM(IF((MES='FEV' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_FEV, SUM(IF((MES='MAR' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_MAR, SUM(IF((MES='ABR' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_ABR, SUM(IF((MES='MAI' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_MAI, SUM(IF((MES='JUN' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_JUN, SUM(IF((MES='JUL' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_JUL, SUM(IF((MES='AGO' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_AGO, SUM(IF((MES='SET' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_SET, SUM(IF((MES='OUT' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_OUT, SUM(IF((MES='NOV' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_NOV, SUM(IF((MES='DEZ' AND TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL_DEZ, SUM(IF((TIPO='VOLUME' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS VOLUMEREAL FROM `apoioorcamento` WHERE "
                    + "`REGIAO` LIKE '%" + nomeregiao
                    + "' AND `PLATAFORMA` LIKE '%" + nomeplataforma
                    + "' AND `FILIAL` LIKE '%" + nomefilial
                    + "' AND `ANO` = " + nomeano
                    + " AND (`AREA` LIKE '" + nomecifexp
                    + "' OR `AREA` LIKE '" + nomecifrec
                    + "' OR `AREA` LIKE '" + nomefob + "')"
                    + "");
            ResultSet rs1 = st.getResultSet();
            while (rs1.next()) {
                idovol = rs1.getInt("ID");
                mes1volplan = rs1.getDouble("VOLUMEPLAN_JAN");
                mes2volplan = rs1.getDouble("VOLUMEPLAN_FEV");
                mes3volplan = rs1.getDouble("VOLUMEPLAN_MAR");
                mes4volplan = rs1.getDouble("VOLUMEPLAN_ABR");
                mes5volplan = rs1.getDouble("VOLUMEPLAN_MAI");
                mes6volplan = rs1.getDouble("VOLUMEPLAN_JUN");
                mes7volplan = rs1.getDouble("VOLUMEPLAN_JUL");
                mes8volplan = rs1.getDouble("VOLUMEPLAN_AGO");
                mes9volplan = rs1.getDouble("VOLUMEPLAN_SET");
                mes10volplan = rs1.getDouble("VOLUMEPLAN_OUT");
                mes11volplan = rs1.getDouble("VOLUMEPLAN_NOV");
                mes12volplan = rs1.getDouble("VOLUMEPLAN_DEZ");
                acumvolplan = rs1.getDouble("VOLUMEPLAN");
                mes1volreal = rs1.getDouble("VOLUMEREAL_JAN");
                mes2volreal = rs1.getDouble("VOLUMEREAL_FEV");
                mes3volreal = rs1.getDouble("VOLUMEREAL_MAR");
                mes4volreal = rs1.getDouble("VOLUMEREAL_ABR");
                mes5volreal = rs1.getDouble("VOLUMEREAL_MAI");
                mes6volreal = rs1.getDouble("VOLUMEREAL_JUN");
                mes7volreal = rs1.getDouble("VOLUMEREAL_JUL");
                mes8volreal = rs1.getDouble("VOLUMEREAL_AGO");
                mes9volreal = rs1.getDouble("VOLUMEREAL_SET");
                mes10volreal = rs1.getDouble("VOLUMEREAL_OUT");
                mes11volreal = rs1.getDouble("VOLUMEREAL_NOV");
                mes12volreal = rs1.getDouble("VOLUMEREAL_DEZ");
                acumvolreal = rs1.getDouble("VOLUMEREAL");
