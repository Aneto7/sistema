 ...
 * @author rosaan
 */
public class ServletOrcamentoFilial extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, DESCRICAO_FILIAL FROM `orcamentocontrole` WHERE "
                    ...
                    + "' GROUP BY DESCRICAO_FILIAL ORDER BY DESCRICAO_FILIAL");
        ...
        for (int i = 0; i < filiais.size(); i++) {
            sb.append(filiais.get(i).getId() + "%" + filiais.get(i).getFilial() + ";");
        }
