 ...
 * @author rosaan
 */
public class ServletFiltroCell extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, SUM(BDG_JAN) AS BDG_JAN, SUM(BDG_FEV) AS BDG_FEV, SUM(BDG_MAR) AS BDG_MAR, SUM(BDG_ABR) AS BDG_ABR, SUM(BDG_MAI) AS BDG_MAI, SUM(BDG_JUN) AS BDG_JUN, SUM(BDG_JUL) AS BDG_JUL, SUM(BDG_AGO) AS BDG_AGO, SUM(BDG_SET) AS BDG_SET, SUM(BDG_OUT) AS BDG_OUT, SUM(BDG_NOV) AS BDG_NOV, SUM(BDG_DEZ) AS BDG_DEZ, SUM(REAL_JAN) AS REAL_JAN, SUM(REAL_FEV) AS REAL_FEV, SUM(REAL_MAR) AS REAL_MAR, SUM(REAL_ABR) AS REAL_ABR, SUM(REAL_MAI) AS REAL_MAI, SUM(REAL_JUN) AS REAL_JUN, SUM(REAL_JUL) AS REAL_JUL, SUM(REAL_AGO) AS REAL_AGO, SUM(REAL_SET) AS REAL_SET, SUM(REAL_OUT) AS REAL_OUT, SUM(REAL_NOV) AS REAL_NOV, SUM(REAL_DEZ) AS REAL_DEZ, " + bdgacumuladomes + " AS BDG_ACUM, " + realacumuladomes + " AS REAL_ACUM FROM `orcamentocontrole` WHERE "
                    ...
                    + "' AND `MAGNITUDE_CONTA` LIKE 'Cellphone"
                    + "'");
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, SUM(IF((MES='JAN' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_JAN, SUM(IF((MES='FEV' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_FEV, SUM(IF((MES='MAR' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_MAR, SUM(IF((MES='ABR' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_ABR, SUM(IF((MES='MAI' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_MAI, SUM(IF((MES='JUN' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_JUN, SUM(IF((MES='JUL' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_JUL, SUM(IF((MES='AGO' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_AGO, SUM(IF((MES='SET' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_SET, SUM(IF((MES='OUT' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_OUT, SUM(IF((MES='NOV' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_NOV, SUM(IF((MES='DEZ' AND TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL_DEZ, SUM(IF((TIPO='CELLPHONE' AND PLANOUREAL='REALIZADO'), VALOR, 0)) AS CELLPHONEREAL FROM `apoioorcamento` WHERE "
            ...
            while (rs1.next()) {
                ...
                mes1volreal = rs1.getDouble("CELLPHONEREAL_JAN");
                mes2volreal = rs1.getDouble("CELLPHONEREAL_FEV");
                mes3volreal = rs1.getDouble("CELLPHONEREAL_MAR");
                mes4volreal = rs1.getDouble("CELLPHONEREAL_ABR");
                mes5volreal = rs1.getDouble("CELLPHONEREAL_MAI");
                mes6volreal = rs1.getDouble("CELLPHONEREAL_JUN");
                mes7volreal = rs1.getDouble("CELLPHONEREAL_JUL");
                mes8volreal = rs1.getDouble("CELLPHONEREAL_AGO");
                mes9volreal = rs1.getDouble("CELLPHONEREAL_SET");
                mes10volreal = rs1.getDouble("CELLPHONEREAL_OUT");
                mes11volreal = rs1.getDouble("CELLPHONEREAL_NOV");
                mes12volreal = rs1.getDouble("CELLPHONEREAL_DEZ");
                acumvolreal = rs1.getDouble("CELLPHONEREAL");
