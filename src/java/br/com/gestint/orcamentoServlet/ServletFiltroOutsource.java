 ...
 * @author rosaan
 */
public class ServletFiltroOutsource extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, SUM(BDG_JAN) AS BDG_JAN, SUM(BDG_FEV) AS BDG_FEV, SUM(BDG_MAR) AS BDG_MAR, SUM(BDG_ABR) AS BDG_ABR, SUM(BDG_MAI) AS BDG_MAI, SUM(BDG_JUN) AS BDG_JUN, SUM(BDG_JUL) AS BDG_JUL, SUM(BDG_AGO) AS BDG_AGO, SUM(BDG_SET) AS BDG_SET, SUM(BDG_OUT) AS BDG_OUT, SUM(BDG_NOV) AS BDG_NOV, SUM(BDG_DEZ) AS BDG_DEZ, SUM(REAL_JAN) AS REAL_JAN, SUM(REAL_FEV) AS REAL_FEV, SUM(REAL_MAR) AS REAL_MAR, SUM(REAL_ABR) AS REAL_ABR, SUM(REAL_MAI) AS REAL_MAI, SUM(REAL_JUN) AS REAL_JUN, SUM(REAL_JUL) AS REAL_JUL, SUM(REAL_AGO) AS REAL_AGO, SUM(REAL_SET) AS REAL_SET, SUM(REAL_OUT) AS REAL_OUT, SUM(REAL_NOV) AS REAL_NOV, SUM(REAL_DEZ) AS REAL_DEZ, " + bdgacumuladomes + " AS BDG_ACUM, " + realacumuladomes + " AS REAL_ACUM FROM `orcamentocontrole` WHERE "
                    ...
                    + "' AND `MAGNITUDE_CONTA` LIKE 'Outsourced Labour"
                    + "'");
            ...
            while (rs1.next()) {
                ...
                mes1volplan = rs1.getDouble("BDG_JAN");
                mes2volplan = rs1.getDouble("BDG_FEV");
                mes3volplan = rs1.getDouble("BDG_MAR");
                mes4volplan = rs1.getDouble("BDG_ABR");
                mes5volplan = rs1.getDouble("BDG_MAI");
                mes6volplan = rs1.getDouble("BDG_JUN");
                mes7volplan = rs1.getDouble("BDG_JUL");
                mes8volplan = rs1.getDouble("BDG_AGO");
                mes9volplan = rs1.getDouble("BDG_SET");
                mes10volplan = rs1.getDouble("BDG_OUT");
                mes11volplan = rs1.getDouble("BDG_NOV");
                mes12volplan = rs1.getDouble("BDG_DEZ");
                acumvolplan = rs1.getDouble("BDG_ACUM");
                mes1volreal = rs1.getDouble("REAL_JAN");
                mes2volreal = rs1.getDouble("REAL_FEV");
                mes3volreal = rs1.getDouble("REAL_MAR");
                mes4volreal = rs1.getDouble("REAL_ABR");
                mes5volreal = rs1.getDouble("REAL_MAI");
                mes6volreal = rs1.getDouble("REAL_JUN");
                mes7volreal = rs1.getDouble("REAL_JUL");
                mes8volreal = rs1.getDouble("REAL_AGO");
                mes9volreal = rs1.getDouble("REAL_SET");
                mes10volreal = rs1.getDouble("REAL_OUT");
                mes11volreal = rs1.getDouble("REAL_NOV");
                mes12volreal = rs1.getDouble("REAL_DEZ");
                acumvolreal = rs1.getDouble("REAL_ACUM");
            }
