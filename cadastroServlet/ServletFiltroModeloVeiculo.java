 ...
 * @author Antonio
 */
public class ServletFiltroModeloVeiculo extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, MODELO, TIPO FROM `veiculos` WHERE "
                    + "`TIPO` LIKE '%" + nometipo
        ...
        for (int i = 0; i < modelos.size(); i++) {
            sb.append(modelos.get(i).getId() + "-" + modelos.get(i).getModelo() + ";");
        }
