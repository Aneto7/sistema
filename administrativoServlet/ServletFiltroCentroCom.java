 ...
 * @author rosaan
 */
public class ServletFiltroCentroCom extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            st.executeQuery("SELECT MIN(ID) AS ID, CENTRO_CUSTO FROM `combustivel` WHERE "
                    + "`PLACA` LIKE '%" + nomeplaca
                    + "' AND `CARTAO` LIKE '%" + nomecartao
                    + "' AND `CONDUTOR` LIKE '%" + nomecondutor
                    + "' AND `CENTRO_CUSTO` LIKE '%" + nomecentro
                    + "' GROUP BY CENTRO_CUSTO ORDER BY CENTRO_CUSTO");
        ...
        for (int i = 0; i < centros.size(); i++) {
            sb.append(centros.get(i).getId() + "-" + centros.get(i).getCentro()+ ";");
        }
